...
function buildNFAFromRegex() {
  ...
  if (!regex) {
    document.getElementById('regex-result').innerHTML = '<div class="result-box r-warn">⚠ Masukkan regex</div>';
    return;
  }
  try {
...
function testNFA() {
  if (!nfaData.states.length) {
    document.getElementById('nfa-result').innerHTML = '<div class="result-box r-warn">⚠ Build NFA terlebih dahulu</div>';
    return;
  }
